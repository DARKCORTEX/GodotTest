				...
				if chunk.size() > 0:
					...
					while "\n" in data_buffer:
						...
						if line.begins_with("data:"):
							json_str = line.trim_prefix("data:").strip_edges()
						elif line.begins_with("{"):
							json_str = line
						elif line.begins_with(",") and line.length() > 1:
							json_str = line.substr(1).strip_edges()
						elif line == "," or line == "[" or line == "]":
							continue # Skip Gemini JSON stream markers
