...
func set_api_key(key: String) -> void:
	# Aggressive cleaning: remove ANY character that isn't standard for an API key ([a-zA-Z0-9_-])
	var regex = RegEx.new()
	regex.compile("[^a-zA-Z0-9_\\-]")
	api_key = regex.sub(key.strip_edges(), "", true)
...
func _send_raw_request(message: String, context: String, history: Array, is_agent: bool = false) -> void:
	...
	var model_to_use := current_model.strip_edges()
	var handler = provider_handlers[api_provider]
	if model_to_use.is_empty():
		model_to_use = handler.get_default_model()

	var request_data: Dictionary = handler.build_request(
	...
	if request_data.has("body"):
		...
		if handler.has_method("should_inject_stream_flag"):
			should_inject = handler.should_inject_stream_flag()
	...
	var headers: PackedStringArray = request_data.get("headers", [])
	if handler.has_method("get_sse_accept_header"):
		var acc = handler.get_sse_accept_header()
		if acc.is_empty():
			# Remove any existing Accept header to let SSEClient use default or none
			for i in range(headers.size() - 1, -1, -1):
				if headers[i].to_lower().begins_with("accept:"):
					headers.remove_at(i)
		else:
			var found = false
			for i in range(headers.size()):
				if headers[i].to_lower().begins_with("accept:"):
					headers[i] = "Accept: " + acc
					found = true
					break
			if not found:
				headers.append("Accept: " + acc)
	...
	# DEBUG: Print the final URL to the Godot console (redacted)
	var debug_url = request_data.get("url", "").replace(api_key, "REDACTED")
	print_rich("[color=yellow][AI Assistant] API Request:[/color] " + debug_url)
