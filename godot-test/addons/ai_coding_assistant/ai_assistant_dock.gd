...
func _load_settings() -> void:
	...
	if config.load("user://ai_assistant_settings.cfg") == OK:
		...
		api_manager.set_api_key(key.strip_edges())
		api_manager.set_provider(prov)
		
		# Migration: force-migrate from problematic models to gemini-1.5-flash
		var cleaned_model = model.strip_edges()
		var needs_migration = prov == "gemini" and (
			cleaned_model.is_empty() or 
			"pro" in cleaned_model.to_lower() or 
			"2.5" in cleaned_model or 
			"latest" in cleaned_model.to_lower()
		)
		
		if needs_migration:
			cleaned_model = "gemini-1.5-flash"
			print_rich("[color=cyan][AI Assistant] Auto-Repair: Migrating model to " + cleaned_model + "[/color]")
			
		if not cleaned_model.is_empty(): 
			api_manager.set_model(cleaned_model)
		api_manager.global_context = context
